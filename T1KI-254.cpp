...
const string SHARED_KEY = "mykey123";
const string SHARED_IV = "my_iv456";
...
void run_server(){
    ...
    if(listenSocket == INVALID_SOCKET){
        cerr << "Error: Gagal membuat socket. Kode error: " << WSAGetLastError() << endl;
        return;
    }
    ...
    if(bind(listenSocket, (SOCKADDR*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR){
        cerr << "Error: Gagal bind socket. Kode error: " << WSAGetLastError() << endl;
        closesocket(listenSocket);
        return;
    }

    if(listen(listenSocket, SOMAXCONN) == SOCKET_ERROR){
        cerr << "Error: Gagal listen. Kode error: " << WSAGetLastError() << endl;
        closesocket(listenSocket);
        return;
    }

    cout << "Server berjalan, menunggu koneksi di port " << DEFAULT_PORT << "..." << endl;

    clientSocket = accept(listenSocket, (SOCKADDR*)&clientAddr, &clientAddrSize);
    if(clientSocket == INVALID_SOCKET){
        cerr << "Error: Gagal accept koneksi. Kode error: " << WSAGetLastError() << endl;
        closesocket(listenSocket);
        return;
    }

    cout << "Klien terhubung! Mulai chat..." << endl;
    closesocket(listenSocket); 

    char recvBuffer[BUFFER_SIZE];
    string plaintext_reply;
    int bytesReceived;

    while(true){
        bytesReceived = recv(clientSocket, recvBuffer, BUFFER_SIZE, 0);
        if(bytesReceived > 0){
            string received_ciphertext(recvBuffer, bytesReceived);
            string decrypted_message = des_decrypt_cbc(received_ciphertext, SHARED_KEY, SHARED_IV);
            
            cout << "\nKlien (Didekripsi): " << decrypted_message << endl;
            print_hex("Klien (Ciphertext) ", received_ciphertext);

        }else if(bytesReceived == 0){
            cout << "Klien terputus." << endl;
            break;
        }else{
            cerr << "Error: Gagal recv. Kode error: " << WSAGetLastError() << endl;
            break;
        }

        cout << "Anda (Plaintext)  : ";
        getline(cin, plaintext_reply);

        if(plaintext_reply == "exit") break;

        string ciphertext_reply = des_encrypt_cbc(plaintext_reply, SHARED_KEY, SHARED_IV);
        
        if(send(clientSocket, ciphertext_reply.c_str(), ciphertext_reply.length(), 0) == SOCKET_ERROR){
            cerr << "Error: Gagal send. Kode error: " << WSAGetLastError() << endl;
            break;
        }
    }
...
void run_client(){
    ...
    cout << "Masukkan IP Address Server (cth: 127.0.0.1 untuk local): ";
    ...
    if(connect(connectSocket, (SOCKADDR*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR){
        cerr << "Error: Gagal connect ke server. Kode error: " << WSAGetLastError() << endl;
        closesocket(connectSocket);
        return;
    }

    cout << "Terhubung ke server! Mulai chat... (ketik 'exit' untuk keluar)" << endl;
    ...
    while(true){
        ...
        if(plaintext_message == "exit") break;

        string ciphertext_message = des_encrypt_cbc(plaintext_message, SHARED_KEY, SHARED_IV);
        
        if(send(connectSocket, ciphertext_message.c_str(), ciphertext_message.length(), 0) == SOCKET_ERROR){
            cerr << "Error: Gagal send. Kode error: " << WSAGetLastError() << endl;
            break;
        }
        ...
        if(bytesReceived > 0){
            ...
            cout << "\nServer (Didekripsi): " << decrypted_message << endl;
            print_hex("Server (Ciphertext) ", received_ciphertext);

        }else if(bytesReceived == 0){
            cout << "Server terputus." << endl;
            break;
        }else{
            cerr << "Error: Gagal recv. Kode error: " << WSAGetLastError() << endl;
            break;
...
int main(){
    ...
    if(wsaErr != 0){
        cerr << "WSAStartup gagal dengan error: " << wsaErr << endl;
        return 1;
    }
    
    if(SHARED_KEY.length() != 8 || SHARED_IV.length() != 8){
        cerr << "Error: SHARED_KEY dan SHARED_IV HARUS 8 karakter." << endl;
        WSACleanup();
        return 1;
    }

    int choice;
    cout << "--- Aplikasi Chat Terenkripsi DES ---" << endl;
    cout << "---------------------------------------" << endl;
    cout << "Kunci yang digunakan: " << SHARED_KEY << endl;
    cout << "IV yang digunakan   : " << SHARED_IV << endl << endl;
    cout << "Pilih mode:" << endl;
    cout << "1. Jalankan sebagai Server (Menerima koneksi)" << endl;
    cout << "2. Jalankan sebagai Klien (Menghubungi server)" << endl;
    cout << "Pilihan(1/2): ";
    cin >> choice;
    cin.ignore(); 
